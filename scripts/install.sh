...
warn()  { printf '\033[0;33m[mgit]\033[0m %s\n' "$*" >&2; }
...
install_via_release() {
  ...
  info "Installing mgit ${TAG} (${OS}/${ARCH}) to ${INSTALL_DIR}..."
...
if command -v mgit >/dev/null 2>&1; then
  ...
  warn "mgit was installed to ${INSTALL_DIR}/${BINARY} but is not on your PATH yet"
fi
